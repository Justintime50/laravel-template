...
2. Change the `session` driver to `database`:
   1. Use `php artisan make:session-table` if not autogenerated
3. Change the `cache` driver to `database`:
   1. Use `php artisan make:cache-table` if not autogenerated
4. Change the `queue` driver to `database`:
   1. Use `php artisan make:queue-table` if not autogenerated
   2. Use `php artisan make:queue-failed-table` if not autogenerated
...
mv src/.env.example src/.env-example && cp src/.env-example src/.env && cp .env-example .env
