...
healthcheck() {
    docker ps | grep -q laravel-template-laravel-template-1
    # docker ps | grep -q laravel-template-laravel-template-db-1
    # docker exec -t laravel-template-laravel-template-db-1 mysql -uroot -ppassword -e "show databases;" &>/dev/null || false
}
